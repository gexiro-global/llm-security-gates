...
machine where the heavy ML dependency is not installed.
"""
import argparse
import json
import sys
...
class ScanConfigError(ValueError):
    """Invalid scanner configuration (e.g. an empty input firewall)."""
...
def _reject_unknown(names, registry):
    ...
    if unknown:
        raise ScanConfigError(f"unknown scanner(s): {unknown}; known: {sorted(registry)}")
...
def _shape(mode, original, sanitized, valid, scores, elapsed_ms):
    ...
    ``scores`` are keyed by scanner name (llm-guard's contract). An empty
    ``valid`` here means "no scanner ran"; callers on the input path forbid that
    upstream, so for output-only (optionally empty) use it means nothing to flag.
    """
    return {
        "mode": mode,
...
def scan_input(text, scanners):
    """Scan an input prompt. FAIL CLOSED: never allow-all.

    Raises ``ScanConfigError`` if no scanners are configured, and ``RuntimeError``
    if the library returns no verdicts despite configured scanners.
    """
    if not scanners:
        raise ScanConfigError("input firewall requires at least one scanner")
    from llm_guard import scan_prompt
    t0 = time.perf_counter()
    sanitized, valid, scores = scan_prompt(scanners, text)
    if not valid:
        raise RuntimeError("input scanners returned no verdicts; refusing to pass")
...
def scan_output_text(prompt, output, scanners, *, allow_empty_output_scanners=False):
    ...
    if not valid:
        raise RuntimeError("output scanners returned no verdicts; refusing to pass")
