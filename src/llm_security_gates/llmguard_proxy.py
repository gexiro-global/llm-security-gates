...
``:4000``). It scans the *entire* inbound message set before forwarding and the
model reply before returning it, blocking either side on policy.
...
from .llmguard_scan import (
    ScanConfigError, build_input_scanners, build_output_scanners,
    parse_scanner_names, scan_input, scan_output_text,
)
...
def _message_text(content):
    """Extract scannable text from a message's content (str or parts list).

    OpenAI content may be a string or a list of parts. We concatenate every
    text part. Non-text parts (e.g. image_url) carry no scanner-readable text
    and are noted so a reviewer can see they were present but unscanned.
    """
    if content is None:
        return ""
    ...
    if isinstance(content, list):
        ...
        for part in content:
            if isinstance(part, dict):
                ...
                elif part.get("type") and part.get("type") != "text":
                    out.append(f"[non-text part: {part.get('type')}]")
            elif isinstance(part, str):
                out.append(part)
        return "\n".join(out)
    # Unknown shape: stringify so it is scanned rather than silently forwarded.
    return str(content)


def _last_user(messages):
    """Return the content text of the last user message, or '' (pure)."""
...
async def chat(request: Request):
    ...
    scan_text = collect_scan_text(messages)

    gin = scan_input(scan_text, _STATE["in"])
    if gin["blocked"]:
