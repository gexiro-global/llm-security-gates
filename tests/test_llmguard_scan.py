...
# ---- FAIL-CLOSED regression: input firewall must never allow-all ----

def test_scan_input_rejects_empty_scanner_list():
    with pytest.raises(ls.ScanConfigError):
        ls.scan_input("anything", scanners=[])


def test_scan_input_rejects_empty_verdicts(monkeypatch):
    mod = types.ModuleType("llm_guard")
    mod.scan_prompt = lambda scanners, text: (text, {}, {})  # no verdicts
    monkeypatch.setitem(sys.modules, "llm_guard", mod)
    with pytest.raises(RuntimeError):
        ls.scan_input("hello", scanners=[object()])  # a scanner is "configured"
